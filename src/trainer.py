...
class Trainer:
    ...
    def train(self):
        ...
        for epoch in (pbar := tqdm(range(self.epochs))):
            train_loss, train_acc = self.train_one_epoch(dataloader=self.train_dataloader, epoch=epoch)
            train_loss_list.append(train_loss)
            train_acc_list.append(train_acc)

            val_loss, val_acc = self.val_one_epoch(dataloader=self.val_dataloader, epoch=epoch)
            val_loss_list.append(val_loss)
            val_acc_list.append(val_acc)
            pbar.set_description(f'Training model [train_loss={train_loss:.4f}, train_acc={train_acc:.4f}] [val_loss={val_loss:.4f}, val_acc={val_acc:.4f}]')
            ...
            if val_loss < best_val:
                torch.save(self.model, self.output_path / f'{self.model_name}.pth')
                best_val = val_loss
